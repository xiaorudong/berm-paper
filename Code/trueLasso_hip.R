...
for (sd in errorsd_choice) {
  for (s in s_vec) {
    ...
    t1 <- Sys.time()
    res_lasso <- Lasso.model(X_mat, y)
    t2 <- Sys.time()
    difft <- difftime(t2, t1, units = "mins")
    
    Lasso <- data.frame(est=res_lasso$coef$Value, VAR=res_lasso$coef$Predictor)
    truth <- data.frame(truecoef=beta, VAR=paste("X", 1:length(beta), sep = ""))
    Lasso <- merge(truth, Lasso, by="VAR")
    Lasso$FROM <- rep("Lasso", nrow(Lasso))
    Lasso <- Lasso[,c("truecoef", "est", "FROM", "VAR")]
    
    filepath <- paste("RDATA/fixed_caret/onlyres_trueLasso_hip_60vars300obs_", t, "_sd", sd, "_", s, ".RData", sep = "")
    save(Lasso, difft, file=filepath)
  }
}
...
for (sd in errorsd_choice) {
  for (s in s_vec) {
    ...
    filepath <- paste("RDATA/fixed_caret/onlyres_trueLasso_hip_500vars300obs_", t, "_sd", sd, "_", s, ".RData", sep = "")
    save(Lasso, difft, file=filepath)
  }
}